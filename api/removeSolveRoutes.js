...
router.post('/remove-solve', async (req, res) => {
    ...
    try {
        ...
        // Check if the problem ID is already present in the 'problems' field
        const userCollection = db.collection(email);
        const exist = await userCollection.findOne({ problems: problemId });
        if (exist) {
            // Update the user's profile with the problem that is being removed
